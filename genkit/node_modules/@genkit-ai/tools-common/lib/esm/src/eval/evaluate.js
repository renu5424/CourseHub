...
import { evaluatorName, generateTestCaseId, getAction, getEvalExtractors, getModelInput, hasAction, isEvaluator, logger, stackTraceSpans, } from '../utils';
...
async function runPromptAction(params) {
    ...
    if (!model) {
        throw new Error('Could not resolve model. Please specify model and try again');
    }
    let state;
    let renderedPrompt;
    try {
...
async function resolveModel(params) {
    ...
    const promptMetadata = actionData?.metadata?.prompt;
