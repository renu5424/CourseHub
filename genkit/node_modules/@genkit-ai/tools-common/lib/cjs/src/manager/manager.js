...
class RuntimeManager {
    ...
    async handleNewRuntime(filePath) {
        try {
            ...
            if (isValidRuntimeInfo(runtimeInfo)) {
                ...
                const fileName = path_1.default.basename(filePath);
                if (await (0, utils_1.checkServerHealth)(runtimeInfo.reflectionServerUrl, runtimeInfo.id)) {
    ...
    async performHealthChecks() {
        const healthCheckPromises = Object.entries(this.filenameToRuntimeMap).map(async ([fileName, runtime]) => {
            if (!(await (0, utils_1.checkServerHealth)(runtime.reflectionServerUrl, runtime.id))) {
                await this.removeRuntime(fileName);
            }
        });
    ...
    async removeRuntime(fileName) {
        ...
        logger_1.logger.debug(`Removed unhealthy runtime ${fileName} from manager.`);
    }
}
